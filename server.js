...
if (!dbConnectionString) {
  console.error('Missing DATABASE_URL environment variable. Signup and other database routes cannot start without it.');
  process.exit(1);
}
...
app.post('/api/signup', async (req, res) => {
  ...
  if (!/^\d{4}$/.test(withdrawPin)) {
    return res.status(400).json({ error: 'Withdraw PIN must be exactly 4 digits.' });
  }
  ...
  try {
    ...
    `;
    const userValues = [fullName.trim(), username.trim(), phone.trim(), email?.trim() || null, dob.trim(), age, passwordHash, withdrawPinHash, termsAccepted];

    const result = await client.query(createUserQuery, userValues);
...
app.get('/api/user/by-id/:userId/transactions', async (req, res) => {
  const { userId } = req.params;
  try {
    await applyApprovedTransactionsForUser(userId);
